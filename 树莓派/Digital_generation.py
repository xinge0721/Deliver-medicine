...
for i in range(5):
    ...
    # 绘制九宫格中的数字（固定模式：1234 1234 0）
    cell_index = 0
    for row in range(rows):
        for col in range(cols):
            # 从模式中获取数字
            digit = pattern[cell_index]
            cell_index += 1
    ...
    # 保存图片到data文件夹
    save_path = os.path.join("data", f"numbers_pattern_3x3_grid{i}.jpg")
    img.save(save_path, "JPEG", quality=95)
    print(f"已生成固定模式数字九宫格图像: {save_path}")